...
-->
  ...
  -->
	...
	<!-- ============================================================= -->
	<!-- MAKE PAGE-TITLE (OLD 7.MAKE-HTML-HEADER  )                   -->
	<!-- ============================================================= -->
	...
	<xsl:template name="make-title">
		<title>
			<xsl:choose>
				...
				<xsl:when test="/_ROOT_/article/front/journal-meta/journal-id
                [@journal-id-type='pubmed']">
					<xsl:value-of
						select="/_ROOT_/article/front/journal-meta/journal-id
                        [@journal-id-type='pubmed']"/>
					<xsl:text>: </xsl:text>
				</xsl:when>
			...
			<xsl:for-each select="/_ROOT_/article/front/article-meta/volume">
				<xsl:text>Vol. </xsl:text>
				<xsl:apply-templates/>
				<xsl:text> </xsl:text>
			</xsl:for-each>

			<xsl:for-each select="/_ROOT_/article/front/article-meta/issue">
				<xsl:text>Issue </xsl:text>
				<xsl:apply-templates/>
				<xsl:text>: </xsl:text>
			</xsl:for-each>

			<xsl:if test="/_ROOT_/article/front/article-meta/fpage">
				<xsl:choose>
					<xsl:when test="../lpage">
						<xsl:text>pp. </xsl:text>
						<xsl:value-of select="/_ROOT_/article/front/article-meta/fpage"/>
						<xsl:text>-</xsl:text>
						<xsl:value-of select="/_ROOT_/article/front/article-meta/lpage"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text>p. </xsl:text>
						<xsl:value-of select="/_ROOT_/article/front/article-meta/fpage"/>
